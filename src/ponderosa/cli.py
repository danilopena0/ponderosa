...
def cmd_episode(args: argparse.Namespace) -> int:
    ...
    short_id = episode.get('short_id', args.episode_id[:8])
    ...
    lines += [
        "| Field | Value |",
        "|-------|-------|",
        f"| **Episode ID** | `{short_id}` |",
        f"| **Full ID** | `{episode['id']}` |",
        f"| **Themes** | {len(episode.get('themes', []))} |",
        f"| **Learnings** | {len(episode.get('learnings', []))} |",
        f"| **Strategies** | {len(episode.get('strategies', []))} |",
        "",
    ]
    ...
    for category in ("themes", "learnings", "strategies"):
        ...
        for i, item in enumerate(items, 1):
            ...
            if score:
                ...
                score_bar = f" `{'█' * filled}{'░' * (10 - filled)}` {score}"

            lines.append(f"### {i}. {name}{score_bar}")
            if desc:
                lines += ["", desc]
            if keywords:
    ...
    lines += ["---", f"*Generated by Ponderosa*", ""]
    md_content = "\n".join(lines)

    # Output to dedicated folder
    if args.output:
        output_path = Path(args.output)
    else:
        enrichments_dir = Path("enrichments")
        enrichments_dir.mkdir(parents=True, exist_ok=True)
        output_path = enrichments_dir / f"{short_id}_{_slugify(title)}.md"
